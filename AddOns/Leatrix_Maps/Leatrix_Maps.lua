	...
	----------------------------------------------------------------------
	-- 	Leatrix Maps 1.13.24 (18th August 2019, www.leatrix.com)
	----------------------------------------------------------------------
	...
	local function SlashFunc(str)
		...
		if str and str ~= "" then
			...
			elseif str == "admin" then
				...
				LeaMapsLC["MapScale"] = 0.9
				LeaMapsLC["FadeMap"] = "On"
				LeaMapsLC["FadeLevel"] = 1.0
				LeaMapsLC["RememberZoom"] = "On"
				LeaMapsLC["ShowCoords"] = "Off"
	...
	eFrame:SetScript("OnEvent", function(self, event, arg1)
		...
		if event == "ADDON_LOADED" and arg1 == "Leatrix_Maps" then
			...
			LeaMapsLC:LoadVarChk("ShowCoords", "Off")					-- Show coordinates
	...
	local resetMapPosBtn = LeaMapsLC:CreateButton("resetMapPosBtn", PageF, "Reset Map Position", "BOTTOMRIGHT", -16, 60, 25)
