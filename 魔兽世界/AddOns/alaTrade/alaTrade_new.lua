...
do	--	MAIN
	...
	do	--	hook tooltip
		...
		local function AddPrice(tip, id, num)
			local name, link, rarity, item_level, _, _, _, stack, _, _, vdp, item_type, _, bind_type = GetItemInfo(id);
			if SET.show_stack_count then
				tip:AddDoubleLine("-", L["STACK_COUNT"] .. ": " .. (stack or 1), 1, 0.5, 0, 1, 1, 1);
			end
