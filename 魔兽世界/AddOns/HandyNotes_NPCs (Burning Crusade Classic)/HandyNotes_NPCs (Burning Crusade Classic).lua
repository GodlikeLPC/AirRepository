...
local Addon = LibStub("AceAddon-3.0"):NewAddon("HandyNotes_NPCs (Burning Crusade Classic)", "AceConsole-3.0", "AceEvent-3.0")
...
local icons = {
	...
	flightmastersUndiscovered = "Interface\\Addons\\HandyNotes_NPCs (Burning Crusade Classic)\\flightmaster_undiscovered.tga",
...
function pluginHandler:OnEnter(uiMapId, coord)
	...
	if db.showVendorData and nodeData["npcID"] and data["vendors"][nodeData["npcID"]] then
		local tooltip = LibQTip:Acquire("HandyNotes_NPCsBCC", 2, "LEFT", "RIGHT")
...
function Addon:OnInitialize()
	self.db = LibStub("AceDB-3.0"):New("HandyNotes_NPCsBurningCrusadeClassicDB", defaults, true)
	db = self.db.profile
...
function Addon:updateStuff()
	HandyNotes:SendMessage("HandyNotes_NotifyUpdate", "HandyNotes_NPCsBCC")
end
...
function Addon:PLAYER_LOGIN()
 ...
 set = function(info, v) db[info[#info]] = v Addon:CheckSettings() HandyNotes:SendMessage("HandyNotes_NotifyUpdate", "HandyNotes_NPCsBCC") end,
 args = {
  mapButton = {
	type = "toggle",
 ...
 HandyNotes:RegisterPluginDB("HandyNotes_NPCsBCC", pluginHandler, options)
 ...
 if db.mapButton then
	...
	local dropDownMenu = LibDD:Create_UIDropDownMenu("HandyNotes_NPCsBCCDropDownMenu", button)
	LibDD:UIDropDownMenu_Initialize(dropDownMenu, HandyNotes_NPCsBCCDropDownMenu, "MENU")
	...
	self.LDB = LibStub:GetLibrary("LibDataBroker-1.1"):NewDataObject("HandyNotes_NPCsBCC", {
		type = "launcher",
		label = "NPCs",
	...
	self.minimapButton:Register("HandyNotes_NPCsBCC", self.LDB, db.minimapButton)
...
function HandyNotes_NPCsBCCDropDownMenu_OnClick(self, arg1, arg2, checked)
	if (arg2) then
		db[arg1] = arg2
	else
		if db[arg1] then
...
function HandyNotes_NPCsBCCDropDownMenu(frame, level, menuList)
	...
	if level == 1 then
		...
		info.arg1 = "show"
		info.func = HandyNotes_NPCsBCCDropDownMenu_OnClick
...
function Addon:CheckSettings()
	if self.db.profile.minimapButton.hide then
		self.minimapButton:Hide("HandyNotes_NPCsBCC")
	else
		self.minimapButton:Show("HandyNotes_NPCsBCC")
	end
