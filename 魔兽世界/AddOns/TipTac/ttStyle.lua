...
local TT_NotSpecified = "???";
local TT_Targeting = BINDING_HEADER_TARGETING;	-- "Targeting"
local TT_Reaction = {
	[0] = "Tapped",				-- 0	No localized string of this
	FACTION_STANDING_LABEL1,	-- 1	Hostile
	...
	FACTION_STANDING_LABEL6,	-- Honered
	FACTION_STANDING_LABEL7,	-- Revered
	FACTION_STANDING_LABEL8,	-- 8	Exalted
	FACTION_STANDING_LABEL5,	-- 9	Player Friendly
	DEAD,						-- 10	Dead
};
...
function ttStyle:GeneratePlayerLines(u,first,unit)
	...
	-- guild
	local guild, guildRank, rankId = GetGuildInfo(unit);
	if (guild) then
		...
		GameTooltipTextLeft2:SetFormattedText(cfg.showGuildRank and guildRank and "%s<%s> %s%s(%s)" or "%s<%s>",guildColor,guild,COL_LIGHTGRAY,guildRank,rankId);
...
function ttStyle:ModifyUnitTooltip(u,first)
	...
	u.isPetWild, u.isPetCompanion = false, false;	--	UnitIsWildBattlePet(unit), UnitIsBattlePetCompanion(unit);
