...
for _, name in next, TooltipList do
	local Tooltip = _G[name];
	if Tooltip ~= nil then
		Tooltip.SetBackdrop = Tooltip.SetBackdrop or function()
		end
		Tooltip.SetBackdropColor = Tooltip.SetBackdropColor or function(self, r, g, b)
			return self.NineSlice:SetCenterColor(r, g, b, a);
		end
		Tooltip.GetBackdropColor = Tooltip.GetBackdropColor or function(self)
			return self.NineSlice:GetCenterColor();
		end
		Tooltip.SetBackdropBorderColor = Tooltip.SetBackdropBorderColor or function(self, r, g, b)
			return self.NineSlice:SetBorderColor(r, g, b, a);
		end
		Tooltip.GetBackdropBorderColor = Tooltip.GetBackdropBorderColor or function(self)
...
local TT_DefaultConfig = {
	...
	reactText = true,
	colReactText0 = "|cffc0c0c0",
	colReactText1 = "|cffff0000",
	colReactText2 = "|cffff3f00",
	colReactText3 = "|cffff7f00",
	colReactText4 = "|cffffff00",
	colReactText5 = "|cff7fff00",
	colReactText6 = "|cff3fff00",
	colReactText7 = "|cff1fff00",
	colReactText8 = "|cff00ff00",
	colReactText9 = "|cff25c1eb",
	colReactText10 = "|cff808080",

	reactColoredBackdrop = false,
	reactColoredBorder = false,
	colReactBack0 = { 0.2, 0.2, 0.2 },
	...
	colReactBack8 = { 0.05, 0.05, 0.05 },
	colReactBack9 = { 0.05, 0.05, 0.05 },
	colReactBack10 = { 0.05, 0.05, 0.05 },
	...
	anchorInCombat = true,					--	whether to use customized layout below
	...
	if_showItemLevel = true,					-- Used to be true, but changed due to the itemLevel issues
	if_showItemId = true,
	...
	--
	left = UIParent:GetWidth() * 0.5 - 57,
	top = UIParent:GetHeight() * 0.5 + 12,
};
...
local tipBackdrop = GAME_TOOLTIP_BACKDROP_STYLE_DEFAULT or { bgFile = "Interface\\Tooltips\\UI-Tooltip-Background", edgeFile = "Interface\\Tooltips\\UI-Tooltip-Border", tile = 1, tileSize = 16, edgeSize = 16, insets = { left = 4, right = 4, top = 4, bottom = 4 } };
...
function tt:GetUnitReactionIndex(unit)
	...
	-- Others
		...
		return UnitReaction(unit, "player") or 3;
	end
end
...
function tt:ApplyUnitAppearance(tip,u,first)
	...
	elseif first then
		tip:SetBackdropBorderColor(1.0, 1.0, 1.0);
	end
...
local function GetAnchorPosition()
	if InCombatLockdown() and cfg.anchorInCombat then
		return "default";
	else
...
function gttScriptHooks:OnUpdate(elapsed)
	if self:GetUnit() == nil then return end
...
local function GTT_SetDefaultAnchor(tooltip,parent)
	...
	if (tooltip ~= gtt) and (gtt_anchorType == "mouse") then
		...
		elseif gtt_anchorType == "default" then
			tooltip:SetPoint("BOTTOMRIGHT", UIParent, "BOTTOMRIGHT", -100, 130);
		else
...
local function GTT_ClearDefaultAnchor(tooltip, parent, anchor, x, y, ignore)
	if not ignore then
		tooltip.default = nil;
	end
end
...
--	end
	...
	hooksecurefunc(gtt, "SetOwner", GTT_ClearDefaultAnchor)
