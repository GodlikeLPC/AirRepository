...
local function BaudAuction_OnLoad(self, enabled)
	...
	local function ChildEnterFunc(self)
		BaudAuctionBrowseEntry_OnEnter(self:GetParent());
	end
	...
	for Index = 1, nEntries do
		...
		Entry:SetScript("OnEnter", BaudAuctionBrowseEntry_OnEnter);
		Entry:SetScript("OnLeave", BaudAuctionBrowseEntry_OnLeave);
		Entry:SetHeight(16);
		...
		Button:SetScript("OnEnter", ChildEnterFunc);
...
function NS.CreateBaudAuctionFrame(enabled)
	...
	Highlight:Hide();
	BaudAuctionProgress = CreateFrame("FRAME", nil, ScrollBox, "BackdropTemplate");
...
if IsAddOnLoaded("Blizzard_AuctionUI") then
			NS.CreateBaudAuctionFrame(true);
else
