		...
		config.options.plugins.sync = {
			sync = {
				...
				args = {
					...
					quiet = config.toggle("静默模式", "Don't send rare information to others", 10),
					party = config.toggle("队伍", "Accept syncs from party members", 20),
					raid = config.toggle("团队", "Accept syncs from raid members", 30),
					guild = config.toggle("公会同步", "Accept syncs from guild members", 40),
					nearby = config.toggle("仅附近", "Only accept syncs from people who are nearby. Information about guild members isn't available, so they'll only count as nearby if they're in your group.", 50),
				},
			},
		}
	end
