	...
	if config then
		...
		local options = {
			general = {
				...
				get = get, set = set,
				args = {
					already = toggle("已发现的", "Announce when we see rares we've already killed / achieved (if known)"),
					dead = toggle("死亡的稀有", "Announce when we see dead rares, if known. Not all scanning methods know whether a rare is dead or not, so this isn't entirely reliable."),
					flash = toggle("屏幕闪烁", "Flash the edges of the screen."),
					instances = toggle("副本中通告", "Show announcements while in an instance"),
				},
			},
			message = {
		...
		if LSM then
			local soundfile = function(enabled_key, order)
				return {
					...
					name = "播放声音", desc = "Choose a sound file to play",
			...
			options.sound = {
				...
				args = {
					...
					sound = toggle("启用", "Play sounds at all!", 10),
					drums = toggle("鼓声", "Underneath it all, the constant drumming", 12),
					soundgroup = toggle("队伍同步音效", "Play sounds from synced mobs from party/raid members", 13),
					soundguild = toggle("公会同步音效", "Play sounds from synced mobs from guild members not in group", 14),
					soundfile = soundfile("sound", 15),
					sound_loop = soundrange(17),
					mount = {type="header", name="", order=20,},
					sound_mount = toggle("坐骑音效", "Play a special sound for mobs that drop a mount", 21),
					soundfile_mount = soundfile("sound_mount", 25),
					sound_mount_loop = soundrange(27),
					boss = {type="header", name="", order=30,},
					sound_boss = toggle("Boss音效", "Play a special sound for mobs that require a group", 31),
