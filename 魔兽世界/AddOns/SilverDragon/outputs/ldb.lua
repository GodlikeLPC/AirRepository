...
function module:SetupDataObject()
	...
	function dataobject:OnEnter()
		...
		if ns.mobsByZone[zone] then
			...
			tooltip:AddHeader("名字", "数量", "最后一次见到")
		...
		if #rares_seen > 0 then
			tooltip:AddHeader("本次游戏发现:")
			tooltip:AddHeader("名字", "区域", "坐标", "时间", "出处")
			...
			tooltip:AddHeader("本次游戏未发现")
		end

		tooltip:AddSeparator()
		local index = tooltip:AddLine("右键打开设置")
		tooltip:SetLineTextColor(index, 0, 1, 1)
		if core.debuggable then
			index = tooltip:AddLine("Shift-右键可查看调试信息")
			tooltip:SetLineTextColor(index, 0, 1, 1)
		end
