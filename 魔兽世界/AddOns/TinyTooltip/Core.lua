...
function addon:GetZone(unit, unitname, realm)
    ...
    elseif (i and t == "party") then
        ...
        -- return C_Map.GetMapInfo(C_Map.GetBestMapForUnit('player')).name;
        return GetZoneText();
    end
end
...
LibEvent:attachTrigger("tooltip.style.init", function(self, tip)
    ...
    -- tip:SetBackdrop(nil)
    if tip.NineSlice ~= nil then tip.NineSlice:Hide() end
    tip.style = CreateFrame("Frame", nil, tip, "BackdropTemplate")
    ...
    tip.style.inside = CreateFrame("Frame", nil, tip.style, "BackdropTemplate")
    ...
    tip.style.outside = CreateFrame("Frame", nil, tip.style, "BackdropTemplate")
