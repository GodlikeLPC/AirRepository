...
local MAJOR, MINOR = "HereBeDragons-Pins-2.0", 999911
...
function worldmapProvider:RemoveAllData()
    if self:GetMap() then
        worldmapProvider.RemoveAllData = worldmapProvider._RemoveAllData;
        return worldmapProvider:_RemoveAllData();
    end
end
function worldmapProvider:_RemoveAllData()
    self:GetMap():RemoveAllPinsByTemplate("HereBeDragonsPinsTemplate")
end

function worldmapProvider:RemovePinByIcon(icon)
    if self:GetMap() then
        worldmapProvider.RemovePinByIcon = worldmapProvider._RemovePinByIcon;
        return worldmapProvider:_RemovePinByIcon();
    end
end
function worldmapProvider:_RemovePinByIcon(icon)
...
function worldmapProvider:RemovePinsByRef(ref)
    if self:GetMap() then
        worldmapProvider.RemovePinsByRef = worldmapProvider._RemovePinsByRef;
        return worldmapProvider:_RemovePinsByRef();
    end
end
function worldmapProvider:_RemovePinsByRef(ref)
    for pin in self:GetMap():EnumeratePinsByTemplate("HereBeDragonsPinsTemplate") do
...
function worldmapProvider:RefreshAllData(fromOnShow)
    if self:GetMap() then
        worldmapProvider.RefreshAllData = worldmapProvider._RefreshAllData;
        return worldmapProvider:_RefreshAllData();
    end
end
function worldmapProvider:_RefreshAllData(fromOnShow)
    self:RemoveAllData()
...
function worldmapProvider:HandlePin(icon, data)
    if self:GetMap() then
        self.HandlePin = self._HandlePin;
        return self:_HandlePin(icon, data);
    end
end
function worldmapProvider:_HandlePin(icon, data)
