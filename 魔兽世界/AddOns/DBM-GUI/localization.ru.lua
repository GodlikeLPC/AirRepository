...
-- Tab: Global Filter
L.Panel_SpamFilter			= "Возможности DBM"
...
L.SpamBlockNoShowTgtAnnounce= "Не показывать текст и не воспроизводить звук для общих объявлений ЦЕЛИ, которые не затрагивают Вас самих, за исключением случаев, когда отмечено, что конкретное предупреждение игнорирует этот фильтр (фильтр выше переопределяет этот)"
...
L.Area_SpamFilter_Timers	= "Глобальные отключения таймера и параметры фильтрации"
...
L.Area_SpamFilter_Misc		= "Разные глобальные отключения и параметры фильтрации"
...
L.FilterInterruptNoteName	= "Фильтровать предупреждения для прерываемых заклинаний (со счетчиком), если предупреждение не содержит Ваше имя в пользов. заметке"
...
L.Panel_HideBlizzard		= "Параметры Blizzard"
