...
Last Update: $Date: 2010-10-28 05:05:48 +0000 (Thu, 28 Oct 2010) $
Website:
Documentation:
SVN: $URL $
Dependencies: UTF8
...
RatingNameToID = {
  ...
  [CR_RESILIENCE_CRIT_TAKEN] = "MELEE_CRIT_AVOID_RATING",
  [CR_RESILIENCE_PLAYER_DAMAGE_TAKEN] = "RANGED_CRIT_AVOID_RATING",--16
  ...
  --[CR_AVOIDANCE] = "MAINHAND_WEAPON_RATING",
  ...
  ["MELEE_CRIT_AVOID_RATING"] = CR_RESILIENCE_CRIT_TAKEN,
  ["RANGED_CRIT_AVOID_RATING"] = CR_RESILIENCE_PLAYER_DAMAGE_TAKEN,
...
local function PlayerHasAura(aura)
   ...
   for i=1,40 do
	local n,_,icon,_,_,dur,x,_,_=UnitBuff("player",i);
		if n and n == aura then
		return UnitBuff("player",i or "") or UnitDebuff("player", i or "")
		end
	end
end
...
local function GetPlayerAuraRankStack(buff)
  ...
  for i=1,40 do
  local name, icon, count, rank  = UnitAura("player", i)
  -- check debuff
  if name and name == buff then
  ...
  local name, rank, _, stack = UnitBuff("player", i)
...
local function IsMetaGemActive(item)
  ...
  -- Start parsing
  --print (item,link,itemSubType)
  tip:ClearLines() -- this is required or SetX won't work the second time its called
  tip:SetHyperlink(link)
  --print ("tip[3]:GetText()",tip:NumLines(),link,tip[3]:GetText())
  if tip:NumLines()~=4 then return end
...
function StatLogic:GetDodgeChanceBeforeDR()
 ...
 --print ("modDodge, drFreeDodge" , modDodge, drFreeDodge)
...
function StatLogic:GetParryChanceBeforeDR()
  ...
  --local modParry = 1
  -- drFreeParry
  --print ("drFreeParry, " , modParry, drFreeParry,GetParryChance())
  --print (self:GetAvoidanceAfterDR("PARRY", modParry, class))
...
function StatLogic:GetAvoidanceAfterDR(avoidanceType, avoidanceBeforeDR, class)
 ...
 -- print("Type , value, class" , avoidanceType,avoidanceBeforeDR, class)
  ...
  elseif avoidanceType == "MELEE_HIT_AVOID" then
	...
	--print ("avoidanceBeforeDR " , C[class] , K[class] , avoidanceBeforeDR)
...
function StatLogic:GetAvoidanceGainAfterDR(avoidanceType, gainBeforeDR)
  ...
  --print ("avoidanceType, gainBeforeDR,GetParryChanceBeforeDR",avoidanceType, gainBeforeDR,self:GetParryChanceBeforeDR())
  ...
  if avoidanceType == "PARRY" then
    ...
    --print ("avoidanceType, modAvoidance , gainBeforeDR",avoidanceType, modAvoidance , gainBeforeDR)
	local newAvoidanceChance = self:GetAvoidanceAfterDR(avoidanceType, modAvoidance + gainBeforeDR) + drFreeAvoidance
    
	--print ("newAvoidanceChance , GetParryChance",newAvoidanceChance , GetParryChance())
  ...
  elseif avoidanceType == "DODGE" then
    ...
    --print ("newAvoidanceChance",avoidanceType, modAvoidance + gainBeforeDR)
	local newAvoidanceChance = self:GetAvoidanceAfterDR(avoidanceType, modAvoidance + gainBeforeDR) + drFreeAvoidance
...
RatingBase = {
  ...
  [CR_CRIT_RANGED] = 14,
  [CR_CRIT_SPELL] = 14,
  --[CR_MULTISTRIKE] = 10, -- hit avoidance
  --[CR_HIT_TAKEN_RANGED] = 10,
  --[CR_HIT_TAKEN_SPELL] = 8,
...
function StatLogic:GetEffectFromRating(rating, id, level, class)
  ...
  --print ("(rating, id, level, class)",rating, id, level, class)
  ...
  --print (type(rating),type(id),RatingNameToID[id],id)
  ...
  if level >= 80 and level <= 85 then
	...
	--print("return ",rating,RatingBase[id],H[level],rating/RatingBase[id]/H[level], RatingIDToConvertedStat[id])
...
BaseDodge = {
  ...
  3.6640, 3.4943, -4.0873, 2.0957, 3.4178, 3.6640, 2.1080, 3.6587, 2.4211, 5.6097, --(3.2.0 to 3.3.5)
...
function StatLogic:GetDodgePerAgi()
  ...
  if level == 80 and DodgePerAgiStatic[class] then
    --print (DodgePerAgiStatic[class], "DODGE per Agi")
  ...
  local a = -A_g*A_b
  if a == 0 then a=1 end --Si Ab est = 0 car A_g = 90-90 (A -A_b)
  
  
  local b = A_g*(D_dr-D_b)-A_b*(D_r+C*k)-C*A_g
  local c = (D_dr-D_b)*(D_r+C*k)-C*D_r
  --RatingBuster:Print("modAgi",modAgi,"a",a, "b",b, "c",c, "D_b",D_b, D_r, "A_b",A_b, "A_g",A_g, C, k)
