...
L.ERR_MATCH_TIMEOUT_BY_WORLD = '匹配超时'
L.ERR_TEAM_NAME_TABOO = '队伍名包含敏感词'
L.ERR_TEAM_NAME_ALREADY_EXIST = '队伍名已存在'
L.ERR_TEAM_NAME_MODIFY_ONCE = '队伍名只能修改一次'
L.ERR_TEAM_NAME_MODIFY_SAME = '队伍名修改前后相同，不能修改'
L.ERR_SOME_MEMBER_ALREADY_IN_GAME = '队伍中有成员已经在其他游戏中'
L.ERR_TEAM_NAME_NOT_LEADER = '不是队长不能修改队伍名'
L.ERR_TEAM_NAME_NOT_VALID = '队伍名不合法，只能包含中文数字和英文字母'
L.ERR_TEAM_NAME_LEN_ERROR = '队伍名为1到7个(中文，数字，英文字母)'
...
L.MATCH_DESC2 = [[快速参与：
-玩家可自由组队，参赛队伍不设固定成员；
-匹配前请检查队伍人数，{{combatCount}}人匹配；
-团长确保有至少1个以上的战网好友空位；
-获得胜利后将获得个人积分和公会积分；]]

L.RULES2 = [[
欢迎大家参与战争游戏插件{{title}}，以下是玩法描述：

一、快速匹配模式规则

    1.在快速匹配模式中，参赛选手可以自由组队，参赛队伍不设固定成员；
    2.每场阿拉希盆地比赛的有效时间上限为2小时，资源先达到2000时为胜利方，或者比赛时长达到2小时后资源点数领先的一方获得比赛胜利；
    3.每场战歌峡谷比赛的有效时间上限为1小时，比夺得3面旗帜的一方为胜利方，或者比赛时长达到1小时后旗帜数领先的一方获得比赛胜利；

二、胜负判定

    1.战歌峡谷：
    a. 当比赛时，按成功夺取对方的旗数判定：当旗数比为3:2、3:1、3:0时，夺得3面旗帜的一方获胜；2:1、2:0时：夺得2面旗帜的一方获胜；2:2、1:1时，后夺得旗帜的一方获胜；1:0时，夺得1面旗帜的一方获胜；0:0时，双方各计负一场；
    b. 队伍创建时必须为一个10人团队，队伍开始匹配时检查人数，人数不足则无法参与匹配；
    c. 比赛时间不足1小时，因双方人数不足导致的战场自动关闭会以队长上传的数据作为判定胜负的标准，队长先退出的一方判负，若在数据上传时间内陆续退出，判断离开战场时的队伍人数，人数少的一方判负；

    2.阿拉希盆地：
    a. 当比赛结束时，先达到2000点资源的一方获得比赛胜利；
    b. 队伍创建时必须为一个15人团队，队伍开始匹配时检查人数，人数不足则无法参与匹配；
    c. 比赛时间不足2小时，因双方人数不足导致的战场自动关闭会以队长上传的数据作为判定胜负的标准，队长先退出的一方判负，若在数据上传时间内陆续退出的，判断离开战场时的队伍人数，人数少的一方判负；

三、积分规则

    1.玩家每获得1场胜利将获得1个人积分，负场不扣分；
    2.玩家每获得1场胜利将为其公会累计1公会积分，负场不扣分；
...
L.WSG_RULES = [[
欢迎大家参与战争游戏插件和战歌峡谷{{title}}，以下是本次赛事的详细规则：
...
二、参赛队伍的创建
    ...
    6.每支队伍可以在自动命名后修改队名，每个赛事最多1次改名；成功改名的队伍将以<公会名>-队名的形式呈现；
    6.同一角色在1个赛事周期内只能进入1支参赛队伍的大名单；
...
五、其他
    ...
    3.当比赛时间不足1小时，因双方人数不足导致战场自动关闭时，会以服务器数据作为判定胜负标准，先退出战场的一方判负，同时退出战场，则判断战场剩余人数，人数少的一方为负，人数相同判为双败；
...
L.AB_RULES = [[
欢迎大家参与战争游戏插件和阿拉希盆地{{title}}，以下是本次赛事的详细规则：

一、赛事规则

    1.每支参赛队伍可以在比赛日的指定时间段内参与最多{{maxRoundCount}}场阿拉希盆地比赛；
    2.每场阿拉希盆地比赛的有效时间上限为2小时。资源先达到2000时为胜利方，或者比赛时长达到2小时后资源点数领先的一方获得比赛胜利；
...
二、参赛队伍的创建
    ...
    5.成功创建的参赛队伍将以<公会名>-1队（2队、3队、4队…..）等编号自动命名；
    6.每支队伍可以在自动命名后修改队名，每个赛事最多1次改名；成功改名的队伍将以<公会名>-队名的形式呈现；
    6.同一角色在同一个1个赛事周期内只能进入1支参赛队伍的大名单；
    7.参赛队伍大名单成员在进行匹配时，必须处于队伍创建时对应的公会内，如有队员在赛事期间退出公会，该队员须在匹配前须重新加入公会，方能正常匹配；

三、参赛队伍排名

    1.排行榜将按照胜场数进行排名，胜场数多的队伍排名靠前；
    2.相同胜场数的队伍，比赛总场次多的排名靠前；
    3.胜场数与总比赛场数均相同，净胜资源点数多的排名靠前；
...
五、其他
    ...
    1.匹配队列提示超时的，不录入比赛战绩，参赛队伍可以重新开始匹配；
    2.对于滥用战场BUG的行为，我们接受相关举报，并在核实后作判负并取消比赛资格的处罚；
    3.比赛时间不足2小时，因双方人数不足导致的战场自动关闭会以队长上传的数据作为判定胜负的标准，队长先退出的一方判负，若在数据上传时间内陆续退出，判断离开战场时的队伍人数，人数少的一方判负；
