...
function Astrolabe:GetWorldMapSize(mapId)
    ...
    if map then
        ...
        for level, floorData in pairs(floors) do
            if floorData then
...
function Astrolabe:GetUnitPosition( unit, noMapChange )
	...
	if ( x <= 0 and y <= 0 ) then
		...
		local lastMapID, lastFloor = GetCurrentMapAreaID(), LuaUtils.DGV.GetCurrentMapDungeonLevel();
		...
		local M, F = GetCurrentMapAreaID(), LuaUtils.DGV.GetCurrentMapDungeonLevel();
	...
	return GetCurrentMapAreaID(), LuaUtils.DGV.GetCurrentMapDungeonLevel(), x, y;
end
...
local minimapRotationOffset = LuaUtils.GetPlayerFacing_dugi();
...
function Astrolabe:PlaceIconOnMinimap( icon, mapID, mapFloor, xPos, yPos )
	...
	if ( minimapRotationEnabled ) then
		minimapRotationOffset = LuaUtils.GetPlayerFacing_dugi();
	end
...
local function harvestMapData( HarvestedMapData )
	...
	if ( not mapData[0] and numFloors == 0 and (LuaUtils.DGV.GetCurrentMapDungeonLevel()) == 1 ) then
		numFloors = 1;
	...
	if ( numFloors > 0 ) then
		for f = 1, numFloors do
			...
			local _, TLx, TLy, BRx, BRy = LuaUtils.DGV.GetCurrentMapDungeonLevel();
