...
local MAJOR, MINOR = "HereBeDragons-Pins-2.0-Dugis", 8
...
function worldmapProvider:HandlePin(icon, data)
    ...
    if uiMapID == WORLD_MAP_ID then
	    ...
	    --Dugis customization
        if data.worldMapShowFlag == HBD_PINS_EVERYWHERE_EXCEPT_WORLD_MAP then
            icon:Hide() 
            return
        end
        ...
        if uiMapID ~= data.uiMapID then
            ...
            if not data.uiMapID then
                ...
                --Dugis customization
                if data.worldMapShowFlag == HBD_PINS_EVERYWHERE 
                or (data.worldMapShowFlag == HBD_PINS_EVERYWHERE_EXCEPT_WORLD_MAP and data.uiMapID ~= WORLD_MAP_ID) then
					show = true
				end
...
--Dugis customization
function HBD:RefreshPin(icon)
    local data = worldmapPins[icon]
    if data then
        worldmapProvider:HandlePin(icon, data)
    end
end
