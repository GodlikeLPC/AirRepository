...
function WMTCollection:Initialize()
	...
	function WMTCollection:Load()
		...
		local function CreateIfNew(trackingType, npcId, ...)
			...
			if x and not PointExists(mapName or WorldMapFrame:GetMapID(), level or 0, trackingType, x, y, npcId) then
				...
				if DGV.Modules.WorldMapTracking then
					DGV.Modules.WorldMapTracking:UpdateTrackingMap()
				end
			end
		end
		...
		local function TrainerOpen()
			...
			or QueryTooltipForGuild(L["Journeyman ([^%s]+)"]) 
			or QueryTooltipForGuild(L["([^%s]+) Trainer Female"])

			--classes
			local englishClassTrainerTypeMatch = BCR[trainerTypeMatch]
			for i = 1, 1000 do
				local result = C_CreatureInfo.GetClassInfo(i)

				if not result then break; end

				if result==trainerTypeMatch then
		...
		local function VendorOpen(_, _,step)
