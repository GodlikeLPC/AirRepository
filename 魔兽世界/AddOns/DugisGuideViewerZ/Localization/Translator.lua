...
do
	...
	function DugisGuideViewer:TranslateQuests()
		...
		for i,_ in pairs(tquests) do
			...
			elseif strmatch(action or "", "[ACT]") then --Localize quest title
				...
				if questid then
					locquest = C_QuestLog.GetQuestInfo(questid)
					if locquest then
						questpartnum 	= tquests[i]:match(L.PART_MATCH)
						if questpartnum then locquest = locquest.." ("..questpartnum..")" end
						tquests[i] = locquest
					else 				
						fails = fails + 1
					end
	...
	function DugisGuideViewer:TranslateQuestObjective(guideIndex)
		if DGV.Localize == 0 then return end
		...
		if questid and questStep and strmatch(action or "", "[NCK]") then
			...
			if objectives then
				local objective = objectives[tonumber(questStep)]
			
				if objective then
					local translatedObj = objective.text
					if translatedObj then	
						if translatedObj:match("(-%s)") then translatedObj =  translatedObj:match("-%s(.*)") end 
						if translatedObj:match("(%sx%s%d)") then translatedObj =  translatedObj:match("(.*)%sx%s%d") end
						
						translatedObj = string.gsub(translatedObj, ": [0-9]*/[0-9]*", "")
		...
		if DugisGuideViewer:IsModuleLoaded("Guides") then
			if ( TicksCounter > 2 ) then
				...
				if fails == 0 then 
					...
					DugisGuideViewer:UpdateSmallFrame(nil, false)
				end
...
function DugisGuideViewer:GetLocalizedNPC(npcId)
	...
	local npcNameEn = DugisNPCsEn and DugisNPCsEn[tonumber(npcId)]
...
function DGV:GetLocalizedObject(objectId)
    objectId = tonumber(objectId)
    local localizations = DGV["Objects_" .. GetLocale()]
    
    if localizations and #localizations > 0 then
        return localizations[objectId]
	end
	
	return DGV.Objects_enUS and DGV.Objects_enUS[objectId]
end
