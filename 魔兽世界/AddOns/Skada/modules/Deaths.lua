...
local __ID_SWING = 6603;
local __ID_INSTANTLY_KILL = 42577;
local __ID_DEATH = 41220;
local __ID_SPIRITOFREDEMPTION = 27827;	--	use name of 20711
...
Skada:AddLoadableModule("Deaths", nil, function(Skada, L)
	...
	local death_spell = __ID_DEATH -- Death
	...
	local function AuraApplied(...)
		...
		if spellId == __ID_SPIRITOFREDEMPTION then -- Spirit of Redemption, Holy priest just died
			local timestamp = ...
	...
	local function Instakill(timestamp, eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, ...)
		-- Instakill events
		local spellId, spellName, spellSchool = ...
		spellId = spellId or __ID_INSTANTLY_KILL
	...
	function mod:Update(win, set)
		...
		for i, player in ipairs(set.players) do
			if player.deaths and #player.deaths > 0 then
				...
				if spellid then
					d.label = player.name .. ": " .. (spellname or GetSpellInfo(spellid) or ("spell:" .. spellid))
				else
					d.label = player.name
				end
			...
			for i, death in ipairs(player.deaths) do
				...
				for j, log in ipairs(death.log) do
					...
					if diff > -60 then
						...
						d.id = nr
						local spellid = log.spellid or __ID_SWING -- "Attack" spell
						local spellname = log.spellname or GetSpellInfo(spellid) or ("spell:" .. spellid)
						local rspellname
